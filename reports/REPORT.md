...
Sources: `guidewire_pose_estimation/results/final_summary.csv` (one row per experiment) and `results/metrics_per_experiment.csv` (per-wire breakdown). Bar chart with 95% CI error bars: Figure 1 (`figures/experiment_comparison.png`). Full per-sample error distribution: Figure 2 (`figures/error_cdf.png`).
